...
func physics_update(_delta: float) -> void:
	...
	# 1. Se o jogador saiu dos limites da escada (can_climb virou false), volta a cair
	if not movement.can_climb:
		state_machine.transition_to("Falling")
		return

	# 2. Na escada NÃO aplicamos gravidade (o personagem fica parado se o jogador não mover)
	...
	# 3. Se desceu até atingir o chão e não está tentando subir, sai da escada e vai para Idle
